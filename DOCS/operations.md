...
rarity (common → UNLIMITED but locked until scanned once, uncommon/rare → UNLOCK, mythic → UNIQUE) and
makes legendary creatures commander-eligible. Adjust the tests' expected catalog total, then:
...
Every new card lowers everyone's collection % until they collect it: the Collector achievements count owned
cards out of the whole catalog.
