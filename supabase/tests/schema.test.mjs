...
console.log('\n# starter pack')
{
  ...
  const unl = await q(`select c.forge_name, c.colors, c.types from public.player_unlocks u join public.cards c on c.id = u.card_id where u.player_id = $1 and c.id <> $2`, [bob, SHIVAN])
  const names = unl.map((x) => x.forge_name).sort()
  check('new account gets exactly the 10 starter creatures', names.join() === [...RED, ...GREEN].sort().join(), names.join())
  check('5 red + 5 green, all creatures', unl.filter((x) => x.colors === 'R').length === 5 && unl.filter((x) => x.colors === 'G').length === 5 && unl.every((x) => /Creature/.test(x.types)))
  ...
  const xc = await q(`select degree_level, (select count(*)::int from public.player_unlocks where player_id = $1) as n from public.profiles where id = $1`, [x])
  check('invalid cohort metadata does not block signup', xc[0].degree_level === null && xc[0].n === 10, JSON.stringify(xc))
